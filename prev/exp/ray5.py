...
# XML string for a 2-DOF ball environment with adjustable obstacles
XML_2DOF = """
...
# Obstacle locations in the environment
...
# Function to generate vectors in the camera frame
...
# Function to transform vectors from camera frame to global frame
...
# Render an initial depth image
renderer.update_scene(data, "fwd")
media.write_image("depth/img.png", renderer.render())

# Initialize MuJoCo data structures for batch rendering
...
vecs_cf = get_vecs_cf(DCAM_PARAMS)
vecs_gf = cf2gf(vecs_cf, dx, 0)

# Define batched raycasting function
f_ray = jax.jit(partial(mjx.ray, geomgroup=None))
in_ax = (None,)*3 + (0,)
