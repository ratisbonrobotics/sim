...
# XML definition of the MuJoCo environment (with added obstacles)
xml_2dof = """
...
num_envs = 8000  # Number of parallel environments for batched depth rendering
duration, framerate = 2, 30  # Video recording parameters
...
def get_vecs_cf(dcam_pars):
    """Generate camera-frame ray vectors based on camera parameters."""
...
def cf2gf(vecs_cf, mjx_data, cam_ind):
    """Transform camera-frame vectors to global frame."""
...
f_ray = jax.jit(partial(mjx.ray, geomgroup=None))  # Jitted raycasting function
in_ax = (None,) * 3 + (0,)
...
f_bdmap = jax.jit(jax.vmap(f_dmap, in_axes=(None,) * 3 + (0,)))  # Batched depth map function
...
# --- Save videos and print rendering performance ---
