...
struct Mat4f {
    float m[4][4];
    __host__ __device__ Mat4f() { for (int i = 0; i < 4; i++) for (int j = 0; j < 4; j++) m[i][j] = (i == j) ? 1.0f : 0.0f; }
    ...
    __host__ __device__ Vec3f transformNormal(const Vec3f& n) const { return Vec3f(m[0][0] * n.x + m[0][1] * n.y + m[0][2] * n.z, m[1][0] * n.x + m[1][1] * n.y + m[1][2] * n.z, m[2][0] * n.x + m[2][1] * n.y + m[2][2] * n.z).normalize(); }
};

__device__ Vec3f barycentric(Vec3f A, Vec3f B, Vec3f C, Vec3f P) {
    Vec3f s[2] = {Vec3f(C.x - A.x, B.x - A.x, A.x - P.x), Vec3f(C.y - A.y, B.y - A.y, A.y - P.y)};
    Vec3f u = s[0].cross(s[1]);
    return std::abs(u.z) > 1e-2 ? Vec3f(1.f - (u.x + u.y) / u.z, u.y / u.z, u.x / u.z) : Vec3f(-1, 1, 1);
}
...
__global__ void rasterize_kernel(Vec3f* projected_vertices, Triangle* triangles, int* triangle_counts, unsigned char* textures, int* tex_widths, int* tex_heights, Mat4f* model_matrices, unsigned char* output, float* zbuffer, int width, int height, int num_objects) {
    ...
    zbuffer[idx] = FLT_MAX;
    Vec3f color(0.2f, 0.2f, 0.2f), light_dir = Vec3f(1, 1, 1).normalize();
    ...
    for (int obj = 0; obj < num_objects; obj++) {
        for (int i = 0; i < triangle_counts[obj]; i++) {
            ...
            for (int j = 0; j < 3; j++) screen_coords[j] = Vec3f((projected_vertices[vertex_offset + i * 3 + j].x + 1.0f) * width / 2.0f, (1.0f - projected_vertices[vertex_offset + i * 3 + j].y) * height / 2.0f, projected_vertices[vertex_offset + i * 3 + j].z);
            ...
            if (frag_depth < zbuffer[idx]) {
                ...
                int tex_x = uv.u * tex_widths[obj], tex_y = (1.0f - uv.v) * tex_heights[obj];
...
void load_obj(const char* filename, std::vector<Triangle>& triangles) {
    ...
    while (std::getline(file, line)) {
        ...
        iss >> type;
        if (type == "v") { Vec3f v; iss >> v.x >> v.y >> v.z; vertices.push_back(v); }
        else if (type == "vt") { Vec2f vt; iss >> vt.u >> vt.v; texcoords.push_back(vt); }
        else if (type == "vn") { Vec3f vn; iss >> vn.x >> vn.y >> vn.z; normals.push_back(vn); }
        else if (type == "f") {
            Triangle tri;
            for (int i = 0; i < 3; i++) {
                int v, vt, vn;
...
int main() {
    ...
    for (int i = 0; i < num_objects; i++) if (!textures[i]) { printf("Failed to load texture for object %d\n", i); return 1; }
    Mat4f model_matrices[num_objects] = {Mat4f(), Mat4f()};
    model_matrices[0].m[0][3] = -1.0f, model_matrices[0].m[2][3] = -3.0f, model_matrices[0].m[0][0] = model_matrices[0].m[2][2] = cos(3.14159f / 4.0f), model_matrices[0].m[0][2] = -(model_matrices[0].m[2][0] = -sin(3.14159f / 4.0f));
    model_matrices[1].m[0][3] = 1.0f, model_matrices[1].m[1][3] = 0.5f, model_matrices[1].m[2][3] = -2.5f, model_matrices[1].m[0][0] = model_matrices[1].m[1][1] = model_matrices[1].m[2][2] = 0.1f;
    Mat4f proj = perspective(3.14159f / 4.0f, (float)width / height, 0.1f, 100.0f);
    std::vector<Vec3f> projected_vertices;
    for (int i = 0; i < num_objects; i++) for (const auto& tri : triangles[i]) for (int j = 0; j < 3; j++) projected_vertices.push_back(proj.transform(model_matrices[i].transform(tri.v[j])));
    ...
    float* d_zbuffer;
    int total_triangles = triangles[0].size() + triangles[1].size(), total_texture_size = (tex_widths[0] * tex_heights[0] + tex_widths[1] * tex_heights[1]) * 3;
    ...
    rasterize_kernel<<<grid_size, block_size>>>(d_projected_vertices, d_triangles, d_triangle_counts, d_textures, d_tex_widths, d_tex_heights, d_model_matrices, d_output, d_zbuffer, width, height, num_objects);
