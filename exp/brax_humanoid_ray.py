...
class Humanoid(PipelineEnv):
    ...
    def _get_obs(self, data: mjx.Data, action: jp.ndarray) -> jp.ndarray:
        """Observes humanoid body position, velocities, and angles using raytracing."""
        ...
        cam_x = xs
        cam_y = ys
        cam_vec = jax.vmap(lambda x, y, m=data.cam_xmat[cam_id]: m @ jp.array([x, y, -f]))(cam_x, cam_y)
        ...
        def render():
            ...
            def fn(_, vec):
                dist, _ = mjx.ray(self.sys, data, cam_pos, vec, geomgroup)
                return None, dist
        ...
        return jp.concatenate([data.qpos[2:], data.qvel, data.cinert[1:].ravel(), data.cvel[1:].ravel(), data.qfrc_actuator, depth.ravel()])
