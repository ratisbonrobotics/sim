    ...
    with open(file_path, 'r') as file:
        for line in file:
            if line.startswith('o '):
                ...
                vertex_offset = sum(len(d['vertices']) for d in drawables)
                texture_coord_offset = sum(len(d['texture_coords']) for d in drawables)
                ...
                for vertex_index in line.split()[1:]:
                    ...
                    v_index = int(indices[0]) - 1 - vertex_offset
                    vt_index = int(indices[1]) - 1 - texture_coord_offset if len(indices) > 1 and indices[1] else -1
                    face.append((v_index, vt_index))
...
def prepare_face_data(faces):
    # Convert faces to a list of tuples with integer values
    prepared_faces = [[(v, t if t != -1 else 0) for v, t in face] for face in faces]
    # Convert to JAX array
    return jnp.array(prepared_faces, dtype=jnp.int32)
...
def create_model_matrices(frame, num_frames):
    ...
    # Create a matrix for each object in the scene
    # We'll assume 3 scene objects for this example, but you can adjust this as needed
    num_scene_objects = 3
    scene_matrices = [create_model_matrix(scale=[0.1, 0.1, 0.1], rotation=[0, i*t, 0], translation=[0.2*i, 0, -3]) 
                      for i in range(num_scene_objects)]
    
    # Add the drone matrix
    drone_matrix = create_model_matrix(scale=[0.1, 0.1, 0.1], rotation=[0, 2*t, 0], translation=[0, 0.5, -2])
    
    # Combine all matrices
    all_matrices = scene_matrices + [drone_matrix]
    
    # Create two sets of matrices for the two output videos
    return jnp.array([all_matrices, 
                      [m.at[0, 3].add(0.5) for m in all_matrices]])  # Shift the second set to the right
...
def main():
    # Load scene model
    scene_obj_file = 'scene0/scene.obj'
    scene_drawables, scene_mtl_file = parse_obj_file(scene_obj_file)
    
    # Load drone model
    drone_obj_file = 'drone.obj'
    drone_drawables, drone_mtl_file = parse_obj_file(drone_obj_file)
    
    # Combine all drawables
    all_drawables = scene_drawables + drone_drawables
    
    # Parse MTL files
    materials = {}
    for obj_file, mtl_file in [(scene_obj_file, scene_mtl_file), (drone_obj_file, drone_mtl_file)]:
        if mtl_file:
            mtl_path = os.path.join(os.path.dirname(obj_file), mtl_file)
            materials.update(parse_mtl_file(mtl_path))
    ...
    for drawable in all_drawables:
        ...
        faces_list.append(prepare_face_data(drawable['faces']))
